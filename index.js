...
console.log(color(figlet.textSync('danzz Botz', {
	...
	console.log(color(`${spc2}           [ Created by: Skyzz ]` ,`${randomcolor}`))
	...
	console.log(color(`${spc3}                    [•]`, `${randomcolor}`), color(`Author      : NothingMD`, `${randomcolor}`))
	console.log(color(`${spc3}                    [•]`, `${randomcolor}`), color(`Base Ori By : Skyzz`, `${randomcolor}`))
	console.log(color(`${spc4}                       < =============================== >`, `${randomcolor}`))
	let { version, isLatest } = await fetchLatestBaileysVersion()
    const danzz = WaConnect({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['Nothing','Chrome','3.0.0'],
        auth: state,
        version
    })
